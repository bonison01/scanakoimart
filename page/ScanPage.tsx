...
type AppState = 'CAPTURING' | 'PREVIEW' | 'ANALYZING' | 'RESULT' | 'ERROR';
...
export default function ScanPage() {
  ...
  const [isSettingsOpen] = useState(false); // Not used in ScanPage, but required by HeaderNavigation
  ...
  const handleSave = (data: ExtractedData) => {
    ...
    navigate('/'); // Go back to HomePage after saving
  };
  ...
  const handleNavigate = (page: 'HOME' | 'SCAN' | 'SETTINGS') => {
    if (page === 'HOME') {
      ...
      navigate('/'); // Or direct to a settings page if one exists
    }
  };
  ...
  return (
    <div className="min-h-screen bg-base-100 text-base-content flex flex-col items-center p-4 sm:p-6">
      ...
      <main className="w-full max-w-3xl bg-base-200 rounded-2xl shadow-xl p-6 sm:p-8 flex items-center justify-center min-h-[400px]">
        ...
        {(appState === 'PREVIEW' || appState === 'ANALYZING' || appState === 'ERROR') && (
          <div className="w-full flex flex-col items-center">
            {imageSrc && (
              <img
                src={imageSrc}
                alt="Captured"
                className="rounded-lg shadow-2xl mb-6 border-4 border-base-300"
              />
            )}
            {appState === 'PREVIEW' && (
